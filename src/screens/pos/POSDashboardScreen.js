...
<div class="ct" style="margin-top:4px">${thankMsg}</div>
  ...
  const handlePrintKOT = useCallback(async () => {
    ...
    try {
      ...
      // Fallback: system print dialog via RNPrint
      const fs = sizeMm <= 58 ? '11px' : '12px';
      const fsSmall = sizeMm <= 58 ? '9px' : '10px';
      const fsBig = sizeMm <= 58 ? '16px' : '18px';
      const fsItem = sizeMm <= 58 ? '12px' : '13px';
      const itemRows = kitchenItems.map(i => {
        const addons = typeof i.addon_details === 'string' ? JSON.parse(i.addon_details || '[]') : (i.addon_details || []);
        const addonLine = addons.length > 0 ? `</tr><tr><td colspan="2" style="font-size:${fsSmall};color:#444;padding-left:8px">+ ${addons.map(a => a.name).join(', ')}</td>` : '';
        const notesLine = i.notes ? `</tr><tr><td colspan="2" style="font-size:${fsSmall};color:#666;padding-left:8px">** ${i.notes}</td>` : '';
        return `<tr><td style="font-size:${fsItem}">${i.name || i.item_name}</td><td style="text-align:center;font-size:${fsItem};font-weight:bold">${i.qty || i.quantity}</td>${addonLine}${notesLine}</tr>`;
      }).join('');
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Courier New',Courier,monospace;font-size:${fs};width:100%;max-width:${sizeMm}mm;padding:3mm;color:#000;word-break:break-word;overflow-wrap:break-word;line-height:1.3}.ct{text-align:center}hr{border:none;border-top:1px dashed #000;margin:5px 0}table{width:100%;border-collapse:collapse;table-layout:fixed}td,th{padding:2px 0;vertical-align:top;font-size:${fs}}@media print{@page{size:${sizeMm}mm auto;margin:0}body{padding:2mm 3mm}html,body{width:${sizeMm}mm}}</style></head><body>
<div class="ct" style="font-weight:bold;font-size:${fsBig}">-- KOT --</div>
<hr>
<table>
...
<hr>
<table><thead><tr><th style="text-align:left">Item</th><th style="text-align:center;width:15%">Qty</th></tr></thead><tbody>${itemRows}</tbody></table>
<hr>
<div class="ct" style="margin-top:4px;font-size:${fsSmall}">--- Kitchen Order Ticket ---</div>
</body></html>`;
      await RNPrint.print({ html, width: sizeMm * 2.83 });
    } catch (err) {
